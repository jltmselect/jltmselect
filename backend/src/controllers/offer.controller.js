...
export const makeOffer = async (req, res) => {
  try {
    ...
    // ===== NEW: DEPOSIT VALIDATION =====
    // Import DepositSettings and BidDeposit models at the top
    // const DepositSettings =
    //   await import("../models/depositSettings.model.js").then((m) => m.default);
    // const BidDeposit = await import("../models/bidDeposit.model.js").then(
    //   (m) => m.default,
    // );

    // Check if deposit system is active
    const depositSettings = await DepositSettings.findOne();

    if (depositSettings?.isActive) {
      // Check if user has paid deposit for this auction
      ...
      if (!deposit) {
        return res.status(400).json({
          success: false,
          message:
            "Deposit required before making offers. Please place a bid first or contact support.",
          requiresDeposit: true, // Add this flag for frontend
        });
      }

      // Optional: Link this offer to the deposit (track which offer used the deposit)
      if (deposit) {
        deposit.metadata.set("offerId", "pending"); // Will update after offer creation
        deposit.metadata.set("offerAmount", offerAmount.toString());
        deposit.metadata.set("offerCreatedAt", new Date().toISOString());
        await deposit.save();
      }
    }
    // ===== END NEW CODE =====
    ...
    // ===== NEW: UPDATE DEPOSIT WITH OFFER ID =====
    if (depositSettings?.isActive) {
      // Find the deposit again and update with the actual offer ID
      const deposit = await BidDeposit.findOne({
        auction: id,
        bidder: buyer._id,
        status: "paid",
      ...
      if (deposit) {
        // Find the newly created offer
        const newOffer = auction.offers.find(
          (o) =>
            o.buyer._id.toString() === buyer._id.toString() &&
            o.amount === offerAmount &&
            o.status === "pending",
        );

        if (newOffer) {
          deposit.metadata.set("offerId", newOffer._id.toString());
          await deposit.save();
        }
      }
    }
  ...
  try {
    ...
    if (auction.bidCount > 0) {
      try {
        ...
        const { cancelAllBidderAuthorizations } =
          await import("../controllers/bidPayment.controller.js");
  ...
  try {
    ...
    const stats = {
      ...
      successRate: (() => {
        ...
        const accepted = allOffers.filter(
          (o) => o.status === "accepted",
        ).length;
      ...
      byAuction: auctions.map((auction) => ({
        ...
        countered: auction.offers.filter((o) => o.status === "countered")
          .length,
        totalValue: auction.offers.reduce(
      ...
      last30Days: (() => {
        ...
        const recentOffers = allOffers.filter(
        ...
        return {
          ...
          totalValue: recentOffers.reduce(
            (sum, offer) => sum + offer.amount,
            0,
          ),
        };
